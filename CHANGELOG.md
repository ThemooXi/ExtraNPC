...
### Added
- Multi-version Paper support: player NPCs use Mannequin (real player model) on Paper 1.21.9+ / 26.1+
- `PlayerNpcProviderFactory` loads Mannequin backend reflectively so the plugin enables on all Paper 1.21+ servers

### Changed
- Player NPCs always use Mannequin — no ArmorStand fallback
...
### Fixed
- Plugin loads safely on all Paper 1.21+ servers without `NoClassDefFoundError` on servers without Mannequin
